...
let insertData =  ( value ) => {
    let _sql = "insert into users set nickname=?,password=?,mobile=?,time=?,unixtime=?,avatar=?"
...
// 修改用户信息
let updateUsers = (value) => {
  let _sql = `update users set nickname=?,email=?,sex=?,about=?,website=? where id=?`
  return query( _sql, value )
}
// 搜索文章
let searchStrArticle = (str) => {
  let _sql = `select * from writer_article where content LIKE '%${str}%'`
  return query( _sql)
}
// 搜索用户
let searchNameUsers = (str) => {
  let _sql = `select * from users where nickname  LIKE '%${str}%'`
  return query( _sql)
}
// 查询所有的作者
let findUsersAuthor = (uid) => {
