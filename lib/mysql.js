...
let createWriterArticle = ( value ) => {
  let _sql = "insert into writer_article set uid=?,writer_id=?,title=?,content=?,time=?,unixtime=?;"
  return query( _sql, value )
}
// 查询某个文集下面的所有文章列表
...
// 查询用户某个文章
let findDataByArticleId = (uid , article_id) => {
  let _sql = `select * from writer_article where uid="${uid}" AND id="${article_id}"`
  return query( _sql)
}
